...
0x41,0x00, // parameters bitmask, set dynamic model, static hold
...
0x00,0x00, // pAcc mask
0x00,0x00, // tAcc mask
0x64,      // Static hold thresh, cm/s (or 1.97 ft/min), 100 cm/s = ~200 ft/min = 0.23mph
0x00,      // DGNSS timeout
...
enum ubxReturn UbloxM8Q_GPS::begin()
{
    // Made it in here as of 30-Oct-2017 @ 3 pm
    static int state = 0;
    enum ubxReturn ret;
...
void UbloxM8Q_GPS::setRate(uint16_t measurement, int navigationSol)
{
...
void UbloxM8Q_GPS::resetODO(void)
{
    sendCommand(resetODOmsg);
    // That was easy!
}
void UbloxM8Q_GPS::setThresh(int speed, uint16_t distance)
{
    if (speed < 1){
	speed = 1;
...
void UbloxM8Q_GPS::sendCommand(uint8_t *msg) // send a ubx format command
{
...
//   the ACK.
enum ubxReturn UbloxM8Q_GPS::writeUBX(uint8_t *msg) // send a ubx format command
{
    int length, i;
...
void UbloxM8Q_GPS::parse(uint8_t *msg, int type, int length)
{
    int typeAcked;
...
void UbloxM8Q_GPS::parsePOSLLH(uint8_t *msg)
{
...
void UbloxM8Q_GPS::parsePVT(uint8_t *msg)
{
    uint8_t *data = &msg[PAYLOAD_IDX];
    ...
    fixquality = data[20];
    fix =  (fixquality == 3); // I arbitrarily decided only valid if 3d
    satellites = data[23];
    longitude =              // Longitude, deg
    longitudeDegrees =
...
void UbloxM8Q_GPS::parseODO(uint8_t *msg)
{
    uint8_t *data = &msg[PAYLOAD_IDX];
...
enum ubxReturn UbloxM8Q_GPS::readUBXmsg(uint8_t *msg, int *type, int *length)
{
...
void UbloxM8Q_GPS::ubxCS( uint8_t *msg, uint8_t *ck_a, uint8_t *ck_b)
{
    int i;
    *ck_a = 0, *ck_b = 0;
...
void UbloxM8Q_GPS::ubxAddCS(uint8_t *msg)
{
    int length = ubx_length(msg);
    ubxCS(msg, &msg[length+6], &msg[length+7]);
}

void UbloxM8Q_GPS::showUbx(uint8_t *msg)
{
    int i;
    int length = ubx_length(msg);
...
int UbloxM8Q_GPS::getInt(uint8_t *m)
{
    return((m[1]*256)+(m[0]));
}

uint16_t UbloxM8Q_GPS::getUInt(uint8_t *m)
{
    return((m[1]*256)+(m[0]));
}

int32_t UbloxM8Q_GPS::getLong(uint8_t *m)
{
    return((m[3]*(0xFFFFFF+1))+(m[2]*(0xFFFF+1))+(m[1]*(0xFF+1))+(m[0]));
}

uint32_t UbloxM8Q_GPS::getULong(uint8_t *m)
{
    return((m[3]*(0xFFFFFF+1))+(m[2]*(0xFFFF+1))+(m[1]*(0xFF+1))+(m[0]));
}

float UbloxM8Q_GPS::getFloat(uint8_t *m)
{
    union ctof_type {
    	uint8_t c[4];
    	float f;
...
void UbloxM8Q_GPS::delayButProcess(uint32_t time)
{
...
void UbloxM8Q_GPS::readVer()
{
    Serial.print("Trying to readVer"); // pjb debug
    int linecnt = 0;
    char c;
    while (linecnt < 10){
...
void UbloxM8Q_GPS::reportError(int state, enum ubxReturn ret)
{
...
int AckedMsg::setAck(int msgType)
{
    for(uint16_t i=0;i<sizeof(ackList)/sizeof(int);i++) {
	     if (!ackList[i]){
	        ackList[i] = msgType;
...
void AckedMsg::clearAck(int msgType)
{
    for(uint16_t i=0;i<sizeof(ackList)/sizeof(int);i++) {
...
bool AckedMsg::isAcked(int msgType)
{
    for(uint16_t i=0;i<sizeof(ackList)/sizeof(int);i++) {
	     if (ackList[i] == msgType){
	        return true;
